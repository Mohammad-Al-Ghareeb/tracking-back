...
exports.createMyOrderCtrl = asyncHandler(async (req, res) => {
  ...
  if (requestedIds.length !== req.body.rawMaterials.length) {
    return res.status(400).json({ message: "Each raw material can only be selected once per order" });
  }

  const materials = await RawMaterial.find({ _id: { $in: requestedIds }, isActive: true });
  if (materials.length !== requestedIds.length) return res.status(400).json({ message: "One or more raw materials are unavailable" });
  ...
  const lines = req.body.rawMaterials.map((line) => {
    ...
    if (available < line.quantity) {
      const err = new Error(`Insufficient stock for ${material.name} ${material.color || ""}`.trim());
      err.statusCode = 409;
      throw err;
    }
    return {
      ...
      const updated = await RawMaterial.findOneAndUpdate(
        {
          ...
          isActive: true,
          $expr: { $gte: [{ $subtract: ["$stockQuantity", "$reservedQuantity"] }, line.quantity] },
        },
        { $inc: { reservedQuantity: line.quantity } },
        { new: true }
      );
      if (!updated) throw Object.assign(new Error("Raw material stock changed. Please review your quantities."), { statusCode: 409 });
      reserved.push(line);
    }
...
exports.getMyOrdersCtrl = asyncHandler(async (req, res) => {
  ...
  const orders = await populateOrder(Order.find(filter)).sort({ createdAt: -1 }).skip((Number(page) - 1) * Number(perPage)).limit(Number(perPage));
...
exports.getAllOrdersCtrl = asyncHandler(async (req, res) => {
  ...
  const orders = await populateOrder(Order.find(filter)).skip((Number(page) - 1) * Number(perPage)).limit(Number(perPage)).sort({ createdAt: -1 });
  const documentCount = await Order.countDocuments(filter);
  res.status(200).json({ orders, pagination: { page: Number(page), perPage: Number(perPage), count: orders.length, documentCount } });
});
...
exports.updateOrderStatusCtrl = asyncHandler(async (req, res) => {
  ...
  if (NEXT_STATUS[order.status] !== nextStatus) {
    return res.status(400).json({ message: `Invalid status transition from ${order.status} to ${nextStatus}` });
  }
  if (order.status === "PENDING") {
    if (!order.employee || !order.expectedFinishDate) return res.status(400).json({ message: "Assign an employee and expected finish date before starting production" });
    if (!order.inventoryConsumed) {
      for (const line of order.rawMaterials) {
        ...
        if (!material || material.reservedQuantity < line.quantity || material.stockQuantity < line.quantity) {
          return res.status(409).json({ message: "Reserved stock is no longer available" });
        }
      }
      for (const line of order.rawMaterials) {
        await RawMaterial.updateOne({ _id: line.rawMaterial }, { $inc: { stockQuantity: -line.quantity, reservedQuantity: -line.quantity } });
      }
...
exports.deleteOrderCtrl = asyncHandler(async (req, res) => {
  ...
  if (order.status === "PENDING" && !order.inventoryConsumed) {
    for (const line of order.rawMaterials) await RawMaterial.updateOne({ _id: line.rawMaterial }, { $inc: { reservedQuantity: -line.quantity } });
  }
